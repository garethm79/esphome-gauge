...
#ifdef USE_GAUGE
#include "esphome/components/gauge/gauge.h"
#endif
...
/////
void fillArc(int cx, int cy, int radius, int thickness, float start, float end, Color color);
...
#ifdef USE_GAUGE
  /** Draw the `gauge` with the top-left corner at [x,y] to the screen.
   *
   * @param x The x coordinate of the upper left corner.
   * @param y The y coordinate of the upper left corner.
   * @param gauge The gauge id to draw
   * @param color_on The color to replace in binary images for the on bits.
   */
  //void gauge(int x, int y, gauge::Gauge *gauge, Color color_on = COLOR_ON);
  void gauge(int x, int y, gauge::Gauge *gauge, Color color_start, Color color_mid, Color color_end, Color color_legend);
