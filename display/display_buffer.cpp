...
void DisplayBuffer::fillArc(int cx, int cy, int radius, int thickness, float start, float end, Color color)
{
  ESP_LOGW("DB", "fillArc %d %d %d %d %f %f", cx, cy, radius, thickness, start, end);

  float _arcAngleMax = 360;
	float sslope = (cos(start/_arcAngleMax * 2 * M_PI) * _arcAngleMax) / (sin(start/_arcAngleMax * 2 * M_PI) * _arcAngleMax) ;
	float eslope = (cos(end/_arcAngleMax * 2 * M_PI) * _arcAngleMax) / (sin(end/_arcAngleMax * 2 * M_PI) * _arcAngleMax);
	...
	for (int x = -radius; x <= radius; x++) {
		for (int y = -radius; y <= radius; y++) {
			...
			if (
				...
				this->draw_pixel_at(cx+x, cy+y, color);
		}
	}
...
#ifdef USE_GAUGE
void DisplayBuffer::gauge(int x, int y, gauge::Gauge *gauge, Color color_start, Color color_mid, Color color_end, Color color_legend) { 
  //gauge->draw(this, x, y, color_on); 
  gauge->draw(this, x, y, color_start, color_mid, color_end, color_legend);
  }
